    ...
    for i in range(24):
        ...
        for j, lt in enumerate(lead_times):
            ...
            if os.path.exists(pred_path):
                ...
                ax = axes[j]
                im = ax.pcolormesh(lon_arr, lat_arr, residual, cmap='bwr', shading='auto')
        ...
        plt.suptitle(f"Residuals for Init: {init_time.strftime('%Y-%m-%d %H:00')} ({args.var_name})", fontsize=24)
        plt.tight_layout(rect=[0, 0.03, 1, 0.98])
        output_path = os.path.join(args.output_dir, f"residual_init_{init_time.strftime('%Y%m%d_%H%M%S')}.png")
