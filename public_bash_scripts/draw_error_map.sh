...
baseline_preds_dir="/tmp3/b12902101/LAM_output_preds/hres_boundary0_no_exact_encoder_res0.25_interp/preds"
...
for plot_mode in "prediction" "residual"; do
    ...
    # HRES: the HRES boundary forecast itself, at each resolution.
    ...
    for resol in "${resols[@]}"; do
        for apply_mode in "direct" "interp"; do
            ...
            python plot_residual.py --var_name "${var_name}" \
                ...
                --preds_dir "/tmp3/b12902101/LAM_output_preds/hres_boundary8_${smooth}_${interp}_${bd_position}_res${resol}_${apply_mode}/preds/" \
                --plot_mode "${plot_mode}" --init_time "${init_time}"
        done
    done
done
